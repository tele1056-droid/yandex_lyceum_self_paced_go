...
func (s *SafeSlice) Append(item int) {
	...
	//и тут мы используем мьютексы, т.к. добавляем в слайс итоговый через append, потому что append может менять сам срез (его длину и иногда указатель на данные), и это уже небезопасно при конкурентном доступе
	s.mx.Lock()
	defer s.mx.Unlock()
